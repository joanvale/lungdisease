...
def upload_xray_image():
	...
	try:
		...
		if report.lower() =="normal":
			...
			symptoms="Symptoms:", "Some of the symptoms of pneumonia are cough, which may produce greenish, yellow or even bloody mucus, fever, shortness of breath rapid, shallow breathing, sharp or stabbing chest pain that gets worse when you breathe deeply or cough, loss of appetite, low energy, and fatigue."           
	except:
		report="Unable to process image"
		symptoms=""

	return render_template('upload.html', filename=filename,report=report,severity=severity,symptoms=symptoms)
